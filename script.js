...
document.addEventListener("DOMContentLoaded", () => {
  ...
  tabButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (selectedTab === "alignment-module" && alignmentChart) {
        setTimeout(() => alignmentChart.resize(), 100);
      }

      if (selectedTab === "map-module" && window.prcsPakistanMap) {
        setTimeout(() => window.prcsPakistanMap.invalidateSize(), 100);
      }
    });
  });
  ...
  const recommendationElement = document.getElementById("priority-recommendation");
  const explanationElement = document.getElementById("recommendation-explanation");
  ...
  function updateRecommendation(score) {
    ...
    if (recommendationElement) {
      ...
      recommendationElement.style.background = recommendationColours[recommendation];
      recommendationElement.style.color = "#ffffff";
    }

    if (explanationElement) {
      explanationElement.textContent = recommendationDescriptions[recommendation];
    }
  }
  ...
  function calculatePriorityScore() {
    ...
    criteria.forEach((criterion) => {
      ...
      const valueLabel = document.getElementById(`${criterion}-value`);
  ...
  function loadInterventionProfile() {
    ...
    const profile = interventionProfiles[interventionSelect.value];
    ...
    criteria.forEach((criterion) => {
      ...
      if (slider) {
        slider.value = profile[criterion];
      }
    });
  ...
  criteria.forEach((criterion) => {
    ...
    if (slider) {
      slider.addEventListener("input", calculatePriorityScore);
    }
  });

  if (interventionSelect) {
    interventionSelect.addEventListener("change", loadInterventionProfile);
  }
  ...
  /* ---------------------------------------------------------
     FULLY INTERACTIVE 7S DIAGNOSTIC
     --------------------------------------------------------- */
  ...
  const sevenSCards = document.querySelectorAll(".s-card");

  function updateSevenSDetails(elementName) {
    const information = sevenSData[elementName];

    if (!information) {
      return;
    }

    const typeElement = document.getElementById("selected-element-type");
    const nameElement = document.getElementById("selected-element-name");
    const descriptionElement = document.getElementById("selected-element-description");
    const currentScoreElement = document.getElementById("selected-current-score");
    const desiredScoreElement = document.getElementById("selected-desired-score");
    const domainContainer = document.getElementById("selected-domain-chips");
    const questionList = document.getElementById("selected-question-list");

    if (typeElement) {
      typeElement.textContent = information.type;
    }
    ...
    if (descriptionElement) {
      descriptionElement.textContent = information.description;
    }

    if (currentScoreElement) {
      currentScoreElement.textContent = `${information.current} / 5`;
    }

    if (desiredScoreElement) {
      desiredScoreElement.textContent = `${information.desired} / 5`;
    }

    if (domainContainer) {
      domainContainer.innerHTML = "";

      information.domains.forEach((domain) => {
    ...
    if (questionList) {
      ...
      information.questions.forEach((question) => {
    ...
    const selectedIndex = chartLabels.indexOf(elementName);

    if (alignmentChart && selectedIndex !== -1) {
      alignmentChart.data.datasets.forEach((dataset, datasetIndex) => {
        dataset.pointRadius = chartLabels.map((_, index) =>
          index === selectedIndex ? 8 : 4
        );

        dataset.pointHoverRadius = chartLabels.map((_, index) =>
          index === selectedIndex ? 10 : 6
        );

        dataset.pointBorderWidth = chartLabels.map((_, index) =>
          index === selectedIndex ? 3 : 1
        );

        dataset.pointBackgroundColor = chartLabels.map((_, index) => {
          if (index === selectedIndex) {
            return "#d4a72c";
          }

          return datasetIndex === 0 ? "#0f8b8d" : "#6f5aa7";
        });
      });
  ...
  sevenSCards.forEach((card) => {
    card.addEventListener("click", () => {
      sevenSCards.forEach((item) => item.classList.remove("active-s-card"));
      card.classList.add("active-s-card");

      const title = card.querySelector("strong");

      if (title) {
        updateSevenSDetails(title.textContent.trim());
      }
    });
  });

  const strategyCard = Array.from(sevenSCards).find((card) => {
    const title = card.querySelector("strong");
    return title && title.textContent.trim() === "Strategy";
  });

  if (strategyCard) {
    strategyCard.classList.add("active-s-card");
  }
  ...
  if (mapElement && typeof L !== "undefined") {
    ...
    window.prcsPakistanMap = pakistanMap;
    ...
    function updateNationalOverview(layer) {
      ...
      const wash = document.getElementById("wash-score");
      const climate = document.getElementById("climate-score");
      const readiness = document.getElementById("readiness-score");
      const direction = document.getElementById("strategic-direction");

      if (name) {
        name.textContent = `Pakistan — ${layerNames[layer]}`;
      }

      if (description) {
        description.textContent = overviewDescriptions[layer];
      }

      if (health) {
        health.textContent = "High";
      }

      if (wash) {
        wash.textContent = "High";
      }

      if (climate) {
        climate.textContent = "Very High";
      }

      if (readiness) {
        readiness.textContent = "Mixed";
      }
      ...
      if (description) {
        description.textContent =
          `${layerNames[selectedLayer]}: ${
            scoreToLabel(province.scores[selectedLayer])
          }. ${province.description}`;
      }

      if (health) {
        health.textContent = scoreToLabel(province.scores.health);
      }

      if (wash) {
        wash.textContent = scoreToLabel(province.scores.wash);
      }

      if (climate) {
        climate.textContent = scoreToLabel(province.scores.climate);
      }

      if (readiness) {
        readiness.textContent = province.readiness;
      }

      if (direction) {
        direction.textContent = province.direction;
      }
    }
    ...
    function updateMapLayer(layer) {
      ...
      provinceMarkers.forEach((item) => {
        const score = item.province.scores[layer];

        item.marker.setRadius(getMarkerRadius(score));
        ...
        item.marker.bindTooltip(
          `
            ...
            ${layerNames[layer]}: ${scoreToLabel(score)}
          `,
          {
            direction: "top",
            sticky: true
    ...
    provinceData.forEach((province) => {
      const initialScore = province.scores.combined;

      const marker = L.circleMarker(province.coordinates, {
        radius: getMarkerRadius(initialScore),
        fillColor: getLayerColour(initialScore, "combined"),
        color: "#ffffff",
        weight: 3,
        opacity: 1,
        fillOpacity: 0.9
      ...
      marker.bindTooltip(
        `
          ...
          Combined Priority: ${scoreToLabel(initialScore)}
        `,
        {
          direction: "top",
          sticky: true
      ...
      marker.on("click", () => {
        ...
        pakistanMap.flyTo(province.coordinates, 6, {
          duration: 0.8
        });
      });
    ...
    layerButtons.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const layer = button.dataset.layer || "combined";
    ...
    resetControl.onAdd = function () {
      ...
      button.title = "Return to Pakistan overview";
      ...
      L.DomEvent.on(button, "click", () => {
        ...
        pakistanMap.flyTo([30.3753, 69.3451], 5, {
          duration: 0.8
        });

        updateNationalOverview(selectedLayer);
      });
